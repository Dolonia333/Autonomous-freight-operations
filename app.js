...
elements.chaseToggle.addEventListener("click", () => {
  threeState.chaseMode = !threeState.chaseMode;
  threeState.cameraSnapped = false;
  updateThreeControls();
});
elements.threeFullscreen.addEventListener("click", () => {
  threeState.fullscreen = !threeState.fullscreen;
  threeState.cameraSnapped = false;
  updateThreeControls();
  setTimeout(resizeThreeView, 40);
});
document.addEventListener("keydown", (event) => {
  if (event.key !== "Escape" || !threeState.fullscreen) return;
  threeState.fullscreen = false;
  threeState.cameraSnapped = false;
  updateThreeControls();
  setTimeout(resizeThreeView, 40);
});
window.addEventListener("resize", () => {
  if (state.mapReady) {
    ...
    focusCamera(Math.max(0, state.currentPhaseIndex), true);
  }
  resizeThreeView();
});
